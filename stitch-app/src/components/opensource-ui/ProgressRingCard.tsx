  ...
  (
    {
      ...
      ringEndColor = "#06b6d4",
      stages = [
        { label: "Design", value: 100, color: "bg-success" },
    ...
    return (
      <div
        ...
        className={cn(
          "w-64 rounded-2xl border border-border-subtle bg-surface p-5 font-sans shadow-lg",
          className,
        )}
        {...props}
      >
        <p
          ...
          className="mb-4 font-mono text-[10px] tracking-widest text-text-muted uppercase"
        >
          {title}
        </p>
        ...
        <div data-slot="progress-ring-card-breakdown" className="space-y-2">
          {normalizedStages.map((stage, index) => (
            <button
              ...
              className={cn(
                "flex w-full cursor-pointer items-center gap-2 rounded-lg px-1 py-0.5 text-left transition-colors hover:bg-surface-soft",
                activeStage === index && "bg-teal-50",
              )}
            >
              <span className="w-16 text-[10px] text-text-secondary">
                {stage.label}
              </span>
              ...
              <span className="w-6 text-right font-mono text-[10px] text-text-muted">
                {stage.value}%
              </span>
            </button>
          ))}
