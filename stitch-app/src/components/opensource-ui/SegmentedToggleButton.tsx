...
>(
  (
    ...
    const [uncontrolledActive, setUncontrolledActive] = useState(defaultIndex);
    ...
    return (
      <div
        ...
        className={cn(
          "relative inline-grid w-fit gap-1 rounded-xl bg-surface-muted p-1 font-sans text-sm font-medium select-none",
      ...
      >
        <span
          aria-hidden
          className={cn(
            "pointer-events-none absolute top-1 bottom-1 left-1 rounded-lg bg-surface",
        ...
        {options.map((option, index) => (
          <button
            ...
            className={cn(
              "relative z-10 min-w-[4.5rem] cursor-pointer rounded-lg px-4 py-2 text-center whitespace-nowrap outline-none focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-text-primary",
              LABEL_MOTION,
              safeActive === index
                ? "text-text-primary"
