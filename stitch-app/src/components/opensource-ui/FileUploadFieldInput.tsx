...
>(function FileUploadFieldInput(
  {
    ...
    hint = "PDF, PPTX, DOCX, or audio up to 50 MB.",
    error = false,
    errorMessage = "Could not upload this file.",
    browseLabel = "Choose file",
    dropLabel = "Drop your file here",
    replaceLabel = "Replace file",
    accept = ".pdf,.pptx,.docx,.mp3,.m4a,.mp4,.wav,.webm,.ogg,.aac,.flac",
    multiple = false,
    maxFiles = 3,
  ...
  return (
    ...
    >
      <label htmlFor={inputId} className="mb-1.5 block w-fit cursor-pointer text-sm font-medium text-text-primary">
        {label}
        {required ? (
      ...
      <div
        className={cn(
          "overflow-hidden rounded-2xl border bg-surface px-5 py-5 transition-[border-color] duration-200",
          showError ? "border-error/30" : "border-border-subtle",
        )}
      >
        {!hasFiles ? (
          <div className="space-y-4">
            <div
              ...
              onDrop={handleDrop}
              className={cn(
                "group relative aspect-[4/3] cursor-pointer overflow-hidden rounded-xl border outline-none ring-0 transition-[border-color,background-color] duration-200 focus:border-text-primary focus:ring-0",
                dragging && !showError && "border-text-primary bg-surface-soft",
                showError && "border-error/40 bg-error-soft",
                !dragging && !showError && "border-border-subtle bg-surface-soft hover:border-border-default hover:bg-surface-soft/80",
            ...
            >
              ...
              >
                {Array.from({ length: 9 }, (_, index) => (
                  <div
                    key={`grid-${index}`}
                    className="border border-border-subtle/90"
                  />
                ))}
              </div>
              ...
              <span
                aria-hidden
                className="absolute top-3 left-3 size-5 border-t-2 border-l-2 border-border-default transition-colors duration-200 group-hover:border-border-subtle0"
              />
              <span
                aria-hidden
                className="absolute top-3 right-3 size-5 border-t-2 border-r-2 border-border-default transition-colors duration-200 group-hover:border-border-subtle0"
              />
              <span
                aria-hidden
                className="absolute bottom-3 left-3 size-5 border-b-2 border-l-2 border-border-default transition-colors duration-200 group-hover:border-border-subtle0"
              />
              <span
                aria-hidden
                className="absolute right-3 bottom-3 size-5 border-r-2 border-b-2 border-border-default transition-colors duration-200 group-hover:border-border-subtle0"
              />

              <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
                <span
                  className={cn(
                    "mb-3 flex size-12 items-center justify-center rounded-xl border border-border-subtle bg-surface text-text-secondary transition-[border-color,transform] duration-200",
                    dragging && "scale-105 border-text-primary text-text-primary",
                    !dragging && "group-hover:border-border-default group-hover:text-text-primary",
                  )}
                >
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {!disabled ? (
                <button
                  ...
                  className="inline-flex h-9 shrink-0 cursor-pointer items-center rounded-lg bg-cta px-4 text-sm font-medium text-white transition-opacity hover:opacity-90"
                >
                  {browseLabel}
                </button>
              ) : null}
            ...
            <div className="flex flex-wrap gap-1.5">
              {acceptTokens.map((token) => (
                <span
                  key={token}
                  className="rounded-full border border-border-subtle bg-surface-soft px-2.5 py-1 text-[10px] font-medium tracking-wide text-text-secondary uppercase"
                >
                  {token}
                </span>
              ))}
          ...
          <div className="space-y-4">
            <div className="relative overflow-hidden rounded-xl border border-border-subtle bg-surface-soft">
              ...
              {!disabled ? (
                <button
                  ...
                  className="absolute top-2.5 right-2.5 flex size-8 cursor-pointer items-center justify-center rounded-full border border-border-subtle bg-surface text-text-secondary transition-colors hover:border-border-default hover:text-text-primary"
                >
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {!disabled ? (
                <button
                  type="button"
                  onClick={openPicker}
                  className="shrink-0 cursor-pointer text-sm font-medium text-text-primary underline-offset-2 hover:underline"
                >
                  {replaceLabel}
                </button>
              ) : null}
          ...
          <div className="space-y-3">
            <ul className="space-y-2">
              {entries.map((entry) => (
                <li
                  key={entry.id}
                  className="flex items-center gap-3 rounded-xl border border-border-subtle bg-surface-soft/80 p-2.5"
                >
                  {entry.previewUrl ? (
                    <div className="size-12 shrink-0 overflow-hidden rounded-lg border border-border-subtle bg-surface">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                    ...
                    <span className="flex size-12 shrink-0 items-center justify-center rounded-lg border border-border-subtle bg-surface text-text-secondary">
                  ...
                  {!disabled ? (
                    <button
                      ...
                      className="flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-lg border border-border-subtle bg-surface text-text-secondary transition-colors hover:border-border-default hover:text-text-primary"
                    >
                      <X size={14} strokeWidth={2} aria-hidden />
                    </button>
                  ) : null}
            ...
            {multiple && entries.length < safeMaxFiles && !disabled ? (
              <button
                type="button"
                onClick={openPicker}
                className="flex h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border-subtle bg-surface text-sm font-medium text-text-primary transition-colors hover:border-border-default hover:bg-surface-soft"
              >
