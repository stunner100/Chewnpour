  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? "border-text-primary bg-surface-soft text-text-primary"
          : "border-transparent text-text-secondary hover:border-border-subtle hover:bg-surface-soft",
      )}
    >
      {option.label}
    </button>
      ...
      >
        <div ref={rootRef} className="relative">
          <button
            ...
            className={cn(
              "flex w-full cursor-pointer items-center justify-between rounded-lg border bg-surface px-3 py-2.5 text-left transition-colors",
              open ? "border-border-default" : "border-border-subtle hover:border-border-default",
            )}
          >
            <span className="min-w-0">
              <span className="block text-[10px] text-text-muted">{label}</span>
              <span className="mt-0.5 block truncate text-[12px] font-medium text-text-primary">
          ...
          {open ? (
            <div
              ...
              className="absolute top-[calc(100%+4px)] right-0 left-0 z-[100] overflow-hidden rounded-lg border border-border-subtle bg-surface py-1 shadow-[0_8px_24px_-8px_rgba(0,0,0,0.1)]"
            >
              {options.map((option) => (
                <FilterSortOptionRow
                  key={option.id}
