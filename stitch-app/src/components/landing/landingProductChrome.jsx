...
export const BrowserFrame = ({ label, url = 'www.chewnpour.com/dashboard', children }) => (
  <figure
  ...
  >
    <div className="flex items-center gap-2 border-b border-[#E5E5EA] bg-[#F9F9F9] px-4 py-2.5">
      ...
      <span className="ml-3 text-xs font-medium text-[#8E8E93]">{url}</span>
    </div>
    {children}
  </figure>
);

export const ScaledStage = ({ children }) => (
  <div
...
export const ProductShell = forwardRef(function ProductShell(
  {
    activeNav,
    hoveredNav = '',
    pressedNav = '',
  ...
  <div
    ref={ref}
    className="relative flex overflow-hidden bg-[#F9F9F9]"
    ...
    <aside className="flex w-60 shrink-0 flex-col border-r border-[#E5E5EA] bg-[#F2F2F7]">
      ...
      <nav className="flex-1 space-y-0.5 px-2">
        {NAV_ITEMS.map((item) => {
          ...
          const isHovered = item.title === hoveredNav;
          const isPressed = item.title === pressedNav;
          return (
            <div
              key={item.title}
              data-demo-nav={item.title}
              data-demo-target={item.title === 'AI Tutor' ? 'tutorNav' : undefined}
              className={`flex h-9 items-center gap-2.5 rounded-lg px-2.5 text-[13px] font-medium transition-transform duration-150 ${
                isActive || isHovered ? 'bg-[#E5E5EA] text-[#0A0A0A]' : 'text-[#6B6B70]'
